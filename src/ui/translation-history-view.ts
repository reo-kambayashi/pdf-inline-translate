...
export class TranslationHistoryView extends ItemView {
	...
	async onOpen() {
		...
		this.historyContainer = container.createEl('div', { cls: 'pdf-inline-translate-history-container' });
	...
	private createHistoryItemElement(item: TranslationHistoryItem) {
		...
		const headerEl = itemEl.createEl('div', { cls: 'pdf-inline-translate-history-item-header' });
		const originalEl = headerEl.createEl('div', { 
			text: item.original.length > 100 ? item.original.substring(0, 100) + '...' : item.original,
