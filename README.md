...
## UI スタイル
- 2025-10-12 時点でポップアップ全体の配色を濃紺ベース（`#020617` / `#0f172a`）とブルーアクセント（`#3b82f6`）に刷新しました。暗色テーマでは黒に近いガラス質のサーフェス、明色テーマでは淡いブルーグレーを基調とし、状態差分は青系の縁とグラデーションで表現します。
- Markdown 表示部は本文を `text-normal`、補助情報を `pdf-inline-translate-muted`（スレート系ブルーグレー）に寄せ、見出し・インジケーター・リンクをアクセントカラーで統一し、黒×青のコントラストを際立たせています。
- 折りたたみアイコンは初期状態で白ベース（`#ffffff`）に固定しつつ、ローディングや成功・警告・エラー時はブルー軸で彩度を切り替えるため視認性とブランド感を両立しています。
