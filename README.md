...
Obsidian の PDF ビューアで選択したテキストを Google Gemini（gemini-2.5-flash-lite）に送信し、インラインで翻訳結果を表示するプラグインです。Gemini API キーはプラグイン設定画面で安全に保存され、翻訳にはレートリミットとリトライ制御が含まれます。

## 主な機能
- PDF ビューアで選択したテキストをワンクリックで翻訳
- 翻訳結果をモーダルで表示し、その場でコピーが可能
...
- 翻訳が完了するとモーダルが表示され、原文と翻訳結果を確認できます。翻訳結果はワンクリックでコピー可能です。
